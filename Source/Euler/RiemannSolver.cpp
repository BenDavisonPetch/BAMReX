    ...
    const amrex::Array4<amrex::Real> &dest)
{
    ...
    const Real p_interm = euler_RP_interm_p(dir, primv_L, primv_R, adiabatic_L, adiabatic_R);
    const Real u_interm = 0.5 * (u_L + u_R) + 0.5 * (euler_riemann_function_one_side(dir, p_interm, primv_R, adiabatic_R) - euler_riemann_function_one_side(dir, p_interm, primv_L, adiabatic_L));

    // primitive values for intermediate states
    GpuArray<Real, MAX_STATE_SIZE> primv_interm_L, primv_interm_R;
    // copy over velocities
    AMREX_D_TERM(primv_interm_L[1] = primv_L[1];, primv_interm_L[2] = primv_L[2];, primv_interm_L[3] = primv_L[3];)
    AMREX_D_TERM(primv_interm_R[1] = primv_R[1];, primv_interm_R[2] = primv_R[2];, primv_interm_R[3] = primv_R[3];)
    ...
    // The speeds of the head and tail of the left and right moving waves. If a wave is a shock, the tail speed is equal to the head speed.
    ...
    if (p_interm > p_L)
    {
        ...
        primv_interm_L[0] = primv_L[0] * (p_ratio + ad_ratio) / (ad_ratio * p_ratio + 1);

        // wave speeds
        speed_L_head = u_L - c_L * std::sqrt(((adiabatic_L+1)*p_interm)/(2*adiabatic_L*p_L) + (adiabatic_L-1)/(2*adiabatic_L));
        ...
        // density
        primv_interm_L[0] = primv_L[0] * std::pow(p_interm / p_L, 1 / adiabatic_L);
        // sound speed behind rarefaction
        const Real c_interm_L = c_L * std::pow(p_interm / p_L, (adiabatic_L-1) / (2*adiabatic_L));
    ...
    if (p_interm > p_R)
    {
        ...
        primv_interm_R[0] = primv_R[0] * (p_ratio + ad_ratio) / (ad_ratio * p_ratio + 1);

        // wave speeds
        speed_R_head = u_R + c_R * std::sqrt(((adiabatic_R+1)*p_interm)/(2*adiabatic_R*p_R) + (adiabatic_R-1)/(2*adiabatic_R));
        speed_R_tail = speed_R_head;
    } else {
        // Right moving rarefaction

        // density
        primv_interm_R[0] = primv_R[0] * std::pow(p_interm / p_R, 1 / adiabatic_R);
        // sound speed behind rarefaction
        const Real c_interm_R = c_R * std::pow(p_interm / p_R, (adiabatic_R-1) / (2*adiabatic_R));
    ...
    ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        ...
        const Real pos = prob_lo[dir_idx] + (dir_idx + 0.5) * dx_arr[dir_idx];
        ...
        if (S <= speed_L_head)
        {
            ...
            for (int n = 0; n < NSTATE; ++n)
                dest(i,j,k,n) = consv_L[n];

        } else if (S <= speed_L_tail)
        {
            // inside fan of left-moving rarefaction
            GpuArray<Real, MAX_STATE_SIZE> primv_fan;
            AMREX_D_TERM(primv_fan[1] = primv_L[1];, primv_fan[2] = primv_L[2];, primv_fan[3] = primv_L[3];)
            const Real tmp = 2 / (adiabatic_L + 1) + (adiabatic_L - 1)/((adiabatic_L+1)*c_L) * (u_L - S);
            primv_fan[0] = primv_L[0] * std::pow(tmp, 2 / (adiabatic_L-1));
            primv_fan[1+dir] = 2 / (adiabatic_L+1) * (c_L + (adiabatic_L-1) * u_L * 0.5 + S);
            primv_fan[1+AMREX_SPACEDIM] = p_L * std::pow(tmp, 2*adiabatic_L / (adiabatic_L - 1));

            const auto consv_fan = consv_from_primv(primv_fan, adiabatic_L);
            ...
            GpuArray<Real, MAX_STATE_SIZE> primv_fan;
            AMREX_D_TERM(primv_fan[1] = primv_R[1];, primv_fan[2] = primv_R[2];, primv_fan[3] = primv_R[3];)
            const Real tmp = 2 / (adiabatic_R + 1) - (adiabatic_R - 1)/((adiabatic_R+1)*c_R) * (u_R - S);
            primv_fan[0] = primv_R[0] * std::pow(tmp, 2 / (adiabatic_R-1));
            primv_fan[1+dir] = 2 / (adiabatic_R+1) * (-c_R + (adiabatic_R-1) * u_R * 0.5 + S);
            primv_fan[1+AMREX_SPACEDIM] = p_R * std::pow(tmp, 2*adiabatic_R / (adiabatic_R - 1));

            const auto consv_fan = consv_from_primv(primv_fan, adiabatic_R);
