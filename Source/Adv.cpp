...
void AmrLevelAdv::advect(
    ...
    AMREX_D_DECL(amrex::FArrayBox &fx, amrex::FArrayBox &fy,
                 amrex::FArrayBox &fz),
    amrex::GpuArray<amrex::Real, BL_SPACEDIM> const &dx, amrex::Real dt)
{
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::z, 1).surroundingNodes(Direction::x),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_xy(i, j, k, phix_y, velx, vely, phix_c, phiy_c, tdtdy); });
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::y, 1).surroundingNodes(Direction::x),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_xz(i, j, k, phix_z, velx, velz, phix_c, phiz_c, tdtdz); });
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::z, 1).surroundingNodes(Direction::y),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_yx(i, j, k, phiy_x, velx, vely, phix_c, phiy_c, tdtdx); });
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::x, 1).surroundingNodes(Direction::y),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_yz(i, j, k, phiy_z, vely, velz, phiy_c, phiz_c, tdtdz); });
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::y, 1).surroundingNodes(Direction::z),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_zx(i, j, k, phiz_x, velx, velz, phix_c, phiz_c, tdtdx); });
    ...
    b = bx;
    amrex::ParallelFor(
        b.grow(Direction::x, 1).surroundingNodes(Direction::z),
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        { flux_zy(i, j, k, phiz_y, vely, velz, phiy_c, phiz_c, tdtdy); });
#endif
    ...
    amrex::ParallelFor(amrex::surroundingNodes(bx, Direction::x),
                       ...
                       {
                           create_flux_x(
                               i, j, k, tfluxx, AMREX_D_DECL(velx, vely, velz),
#if (AMREX_SPACEDIM == 3)
                               phix_c, phiy_z_c, phiz_y_c, hdtdy, hdtdz);
#else
                           phix_c, phiy_c, hdtdy);
#endif
                       });
    ...
    amrex::ParallelFor(amrex::surroundingNodes(bx, Direction::y),
                       ...
                       {
                           create_flux_y(
                               i, j, k, tfluxy, AMREX_D_DECL(velx, vely, velz),
#if (AMREX_SPACEDIM == 3)
                               phiy_c, phix_z_c, phiz_x_c, hdtdx, hdtdz);
#else
                           phiy_c, phix_c, hdtdx);
#endif
                       });
    ...
    amrex::ParallelFor(amrex::surroundingNodes(bx, Direction::z),
                       [=] AMREX_GPU_DEVICE(int i, int j, int k)
                       {
                           create_flux_z(i, j, k, tfluxz, velx, vely, velz,
                                         phiz_c, phix_y_c, phiy_x_c, hdtdx,
                                         hdtdy);
                       });
#endif
    ...
    amrex::ParallelFor(AMREX_D_DECL(nbx[0], nbx[1], nbx[2]),
                       AMREX_D_DECL([=] AMREX_GPU_DEVICE(int i, int j, int k)
