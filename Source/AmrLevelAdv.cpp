...
AmrLevelAdv::initData ()
{
  ...
  for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
  {
    ...
    // The Array4 can be accessed, on CPU and GPU, through its indices, e.g. arr(i,j,k)
    // This determines and Array4 of the initial data which exists on our
    // current multifab
    const Array4<Real>& phi = S_new.array(mfi);

    // Populate MultiFab data
    // The ParallelFor is a loop structure that works on CPU (as a regular loop)
    // or on GPU (assigning things appropriately)
    ParallelFor(box, [=] AMREX_GPU_DEVICE ( int i, int j, int k) noexcept
		     {
		       ...
		       const GpuArray<int, AMREX_SPACEDIM> idx{AMREX_D_DECL(i, j, k)};
		       		       
		       for(unsigned int d = 0; d < AMREX_SPACEDIM; ++d)
		       {
			 Real posn = prob_lo[d] + (idx[d] + Real(0.5)) * dx[d];
			 r2 += posn*posn;
		       }
                      ...
                      int  /*ncycle*/)
{
  ...
  for (int d = 0; d < amrex::SpaceDim ; d++)   
  {
    ...
    for (MFIter mfi(Sborder, true); mfi.isValid(); ++mfi)
    {
      ...
      ParallelFor(bxGrow, [=] AMREX_GPU_DEVICE ( int i, int j, int k) noexcept
		      {
			// Conservative flux for the first-order
			// backward difference method
			fluxArr(i,j,k) = v * arr(i-iOffset, j-jOffset, k-kOffset);
		      });
      ...
      // 	  {
      // 	    amrex::Print() << " (i,j,k) = (" << i << ", " << j << ", " << k << "), flux = " << fluxArr(i,j,k) << " vel = " << vel[d] << " arr = " << arr(i-iOffset, j-jOffset, k-kOffset) << " d = " << d << std::endl;
      // 	  }
      // 	}
      // }
      ...
      ParallelFor(bx, [=] AMREX_GPU_DEVICE ( int i, int j, int k) noexcept
			...
			arr(i,j,k) = arr(i,j,k)
			  - (dt / dx[d]) * (fluxArr(i+iOffset, j+jOffset, k+kOffset)
					    - fluxArr(i,j,k));
		      });
