...
for method in METHODS:
    fastest_runs[method] = {}
    for imgs, mgs in enumerate(MAX_GRID_SIZES):
        fastest_runs[method][mgs] = {}
        for mpi_num_ranks in MPI_RANKS:
            infile_pattern = LOGS[imgs].format(METHOD=method, MPI_NUM_RANKS=mpi_num_ranks)
            ...
            if max(runtimes) - min(runtimes) > runtimes[iminrun]*0.05:
                ...
                print(runtimes)
                print()
            fastest_runs[method][mgs][mpi_num_ranks] = profiledata[iminrun]
...
for imethod, method in enumerate(METHODS):
    for mgs in MAX_GRID_SIZES:
        runtimes = [fastest_runs[method][mgs][nmpi].run_time for nmpi in MPI_RANKS]
        speedups = [fastest_runs[method][mgs][1].run_time/fastest_runs[method][mgs][nmpi].run_time for nmpi in MPI_RANKS]
        label = f"{METHOD_NAMES[imethod]}, MGS = {mgs}"
        ax[0].plot(MPI_RANKS, runtimes, label=label)
        ax[1].plot(MPI_RANKS, speedups, label=label)
...
fig.savefig("outputs/profiling/total_runtime_mgscomp.pdf", bbox_inches="tight")
