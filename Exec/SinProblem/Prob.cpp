...
void initdata(MultiFab &S_tmp, const Geometry &geom)
{
    ...
    bool exact_soln_same_resolution = false;
    {
        ParmParse pp2;
        pp2.get("stop_time", stop_time);
        pp2.query("exact_soln_same_resolution", exact_soln_same_resolution);
    }
    Real p = 1;
    {
        ParmParse pp("prob");
    ...
    if (exact_soln_same_resolution)
    {
        write_exact_solution(stop_time, p, adiabatic, epsilon,
                             geom.Domain().hiVect()[0]
                                 - geom.Domain().loVect()[0]+1);
    }
    else
    {
        write_exact_solution(stop_time, p, adiabatic, epsilon, 1000);
    }
                          ...
                          int resolution)
{
    ...
    for (MFIter mfi(mf, true); mfi.isValid(); ++mfi)
    {
        ...
        ParallelFor(bx,
                    ...
                    {
                        const auto &consv = consv_from_primv(
                            { density_cell_avg(i, dx[0], prob_lo[0], time),
                              AMREX_D_DECL(1, 0, 0), p },
                            adiabatic, epsilon);
                        for (int n = 0; n < EULER_NCOMP; ++n)
                            arr(i, j, k, n) = consv[n];
