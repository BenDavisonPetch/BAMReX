...
{
    Real density_bg, density_packet, u_bg, v_bg, w_bg, pressure_bg;
    Real side_length;
    Real x_c, y_c;
...

    {
        ...
        pp.get("density_packet", density_packet);
        pp.get("u_bg", u_bg);
        pp.get("v_bg", v_bg);
        pp.get("w_bg", w_bg);
        ...
        pp.get("x_c", x_c);
        pp.get("y_c", y_c);
        pp.get("side_length", side_length);
    ...
    // initial conditions
    init_exact_solution(0, S_tmp, geom, density_bg, density_packet, u_bg, v_bg,
                        w_bg, pressure_bg, x_c, y_c, side_length, adiabatic,
                        epsilon);

    // Write exact solution
    MultiFab S_exact(S_tmp.boxArray(), S_tmp.DistributionMap(), EULER_NCOMP,
                     0);
    init_exact_solution(stop_time, S_exact, geom, density_bg, density_packet,
                        u_bg, v_bg, w_bg, pressure_bg, x_c, y_c, side_length,
                        adiabatic, epsilon);
        ...
        for (MFIter mfi(S, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(
                ...
                [=] AMREX_GPU_DEVICE(int i, int j, int k)
                {
                    ...
                    const Real  density = (fabs(x) < 0.5 * side_length
                                          && fabs(y) < 0.5 * side_length)
                                              ? density_packet
                                              : density_bg;
                    const auto &consv   = consv_from_primv(
                          { density, AMREX_D_DECL(u_bg, v_bg, w_bg),
                            pressure_bg },
                          adiabatic, epsilon);
